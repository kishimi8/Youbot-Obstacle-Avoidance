...
void att_potential_field(double dist_att, Point_2 goal, Point_2 ctrl_pt, Matrix &dU);
...
void planner(Listener &p, Polygon_2 pgn_1, Polygon_2 pgn_2, Polygon_2 pgn_3, Polygon_2 pgn_4, Point_2 goal, Matrix &u)
{
...
Matrix dU_att_fl(2,1,1.0);
att_potential_field(dist_att_fl,goal, fl, dU_att_fl);
 ...
 // Forces in Workspace
 // dU_att = dU_att_fl+dU_att_fr+dU_att_bl+dU_att_br;
 // dU_rep = 
...
void att_potential_field(double dist_att, Point_2 goal, Point_2 ctrl_pt, Matrix &dU_att)
{
  double dist_rep;
  double att_gain = 2;
  double att_thresh = 0.5;
//  Matrix dU_att,
 Matrix U_att;
  ...
  q_qgoal[1][1] = ctrl_pt.x() - goal.x();
  q_qgoal[2][1] = ctrl_pt.y() - goal.y();
  
  if (dist_att < att_thresh) 
		{
   U_att =  0.5 * att_gain * pow(dist_att,2);
//   dU_att = att_gain * q_qgoal;
   q_qgoal[1][1] =(double)    q_qgoal[1][1] *att_gain;
   q_qgoal[2][1] = (double)      q_qgoal[2][1] *att_gain;
   dU_att = q_qgoal;
 		}
  else
		{
   ...
   q_qgoal[1][1] = (double)(   q_qgoal[1][1] *att_gain*att_thresh)/dist_att;
   q_qgoal[2][1] =  (double)(   q_qgoal[2][1] *att_gain*att_thresh)/dist_att;
   dU_att = q_qgoal;
...
while(ros::ok())
 { 
  ...
  pub.publish(vel);
  planner(pose,pgn_1,pgn_2,pgn_3,pgn_4, goal, command);
  std::cout<<"came out safely"<<std::endl;
  vel.linear.x = command[1][1];
  vel.linear.y = command[2][1];
  vel.linear.z = command[3][1];
loop_rate.sleep();
}
