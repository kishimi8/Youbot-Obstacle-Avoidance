...
Vector_2 rep_potential_field(double dist_rep, Point_2 nearest, Point_2 ctrl_pt);
...
Vector_3 planner(Listener &p, Polygon_2 pgn_1, Polygon_2 pgn_2, Polygon_2 pgn_3, Polygon_2 pgn_4, Point_2 goal)
{
  ...
  dist_rep_fl = dst_from_obs(fl, pgn_1,pgn_2,pgn_3,pgn_4,nearest_fl);
  dist_rep_fr = dst_from_obs(fr, pgn_1,pgn_2,pgn_3,pgn_4,nearest_fr);
  dist_rep_bl = dst_from_obs(bl, pgn_1,pgn_2,pgn_3,pgn_4,nearest_bl);
  dist_rep_br = dst_from_obs(br, pgn_1,pgn_2,pgn_3,pgn_4,nearest_br);
...
//  OR
 ...
 // Repulsive Potential Field
  ...
  Vector_2 dU_rep_fl(1,1), dU_rep_fr(1,1), dU_rep_bl(1,1), dU_rep_br(1,1);
    dU_rep_fl = rep_potential_field(dist_rep_fl,nearest_fl, fl);
    dU_rep_fr = rep_potential_field(dist_rep_fr,nearest_fr, fr);
    dU_rep_bl = rep_potential_field(dist_rep_bl,nearest_bl, bl);
    dU_rep_br = rep_potential_field(dist_rep_br,nearest_br, br);
...
//std::cout<<u1<<"\t"<<u2<<"\t"<<u3<<std::endl;
    ...
    u3 = (j1.x()*dU_rep_fl.x()+ j1.y()*dU_rep_fl.y()) + (j2.x()*dU_rep_fr.x()+ j2.y()*dU_rep_fr.y()) + (j3.x()*dU_rep_bl.x()+ j3.y()*dU_rep_bl.y()) + (j4.x()*dU_rep_br.x()+ j4.y()*dU_rep_br.y());
...
Vector_2 rep_potential_field(double dist_rep, Point_2 nearest, Point_2 ctrl_pt)
{
  ...
  a= ctrl_pt.x() - nearest.x();
  b= ctrl_pt.y() - nearest.y();
  ...
  if (dist_rep < rep_thresh) 
     ...
     std::cout<<"Near"<<std::endl;
      a = (double) rep_gain * Dq.x()/dist_rep;
      b = (double) rep_gain * Dq.y()/dist_rep;
 		}
  else
		{
     a =0;
  ...
  Vector_2 dU_rep(a,b);
//std::cout<<dU_rep<<std::endl;
//std::cout<<nearest<<std::endl;
//std::cout<<dist_rep<<std::endl;
return dU_rep;
}
