...
public:
 float x;
 float y;
 float thx; float thy; float thz; float thw;
 float x_fl; float y_fl; 
 float x_bl; float y_bl; 
 float x_fr; float y_fr; 
...
float crpdt(Point_2 a, Point_2 b);
float absolute(Point_2 a);

float dst_pt_line(Point_2 p1, Point_2 p2,Point_2 pt )
{
float distance;
//Point_2 intermed1(p2.x() - p1.x(), p2.y() - p1.y());
//Point_2 intermed2(pt.x() - p1.x(), pt.y() - p1.y());
//distance = abs(crpdt(intermed1,intermed2));
//distance = distance/absolute(intermed1);
...
//std::cout<<result<<std::endl;
 ...
 float absolute(Point_2 a)
{
float result = pow((pow(a.x(),2) + pow(a.y(),2)) , 0.5);
//std::cout<<result<<std::endl;
return result;
}


void dst_pt_segment(Segment_2 line,Point_2 pt )
{
std::cout << "sqdist(Segment_2(p,q), m) = "
          << CGAL::squared_distance(line,pt) << std::endl;
}

 float dst_pt_pgn(Polygon_2 pgn, Point_2 pt)
{
...
distance = dst_pt_line(o11,o12,m);
